<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
        <h3 class="flex items-center gap-2 font-bold text-indigo-600 mb-6 uppercase tracking-wider text-sm italic">
            <span class="p-1 bg-indigo-100 rounded">⬇️</span> Sorgente
        </h3>
        <div class="space-y-4">
            ...
            <button type="button" @click="testConnection('source')" class="text-xs font-bold text-indigo-600 hover:text-indigo-800 underline uppercase tracking-tighter">Testa Sorgente</button>
        </div>
    </div>

    <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
        <h3 class="flex items-center gap-2 font-bold text-emerald-600 mb-6 uppercase tracking-wider text-sm italic">
            <span class="p-1 bg-emerald-100 rounded">⬆️</span> Destinazione
        </h3>
        <div class="space-y-4">
            ...
            <button type="button" @click="testConnection('dest')" class="text-xs font-bold text-emerald-600 hover:text-emerald-800 underline uppercase tracking-tighter">Testa Destinazione</button>
        </div>
    </div>
</div>
...
<div x-data="{ open: false }" class="mt-6">
    <button type="button" @click="open = !open" class="flex items-center gap-2 text-sm font-bold text-slate-500 hover:text-indigo-600 transition outline-none">
        ...
        <span>Opzioni Avanzate (Filtri e Limiti)</span>
    </button>
    ...
    <div x-show="open" x-cloak class="mt-4 p-6 bg-white border border-slate-200 rounded-2xl grid grid-cols-1 md:grid-cols-2 gap-6 animate-fade-in">
        <div>
            <label class="block text-[10px] font-black uppercase text-slate-400 mb-2 tracking-widest">Messaggi ultimi X giorni</label>
        ...
        <div>
            <label class="block text-[10px] font-black uppercase text-slate-400 mb-2 tracking-widest">Escludi Cartelle (separate da virgola)</label>
        ...
        <div class="md:col-span-2 flex items-center gap-3 bg-indigo-50 p-3 rounded-lg border border-indigo-100">
            ...
            <label for="dryrun" class="text-sm text-indigo-900 font-bold cursor-pointer uppercase tracking-tighter">Simulazione (Dry-run): non trasferire realmente i messaggi</label>
        </div>
    </div>
</div>